...
- 不在申请中提交 `credential_id`、任意目标 URL 或认证头；
- 执行前确认申请和 Grant 都是 `APPROVED`；
- 执行请求只发一次，失败或结果不确定也不重试；
- 保持心跳并用正确 outcome 结束任务，接口成功返回后再停止心跳。

服务端仍会独立校验 Token、Agent、任务、默认凭证、冻结操作和一次性 Grant。即使 Claude Code 构造了错误请求，也不应绕过人工审批或获得源凭证。
...
2. 调用控制面的 POST /v1/agent/tasks，body 为 {}，保存返回的 task_id。
3. 建立任务后立即启动后台心跳：每 15 秒调用一次 POST /v1/agent/tasks/{task_id}/heartbeat，body 为 {}。等待我审批时也必须继续，不要输出心跳响应或 Agent Token。
4. 调用控制面的 POST /v1/agent/authorizations，提交以下含义完全一致的 JSON：task_id 使用刚建立的任务，target_id 使用 claude-demo 的目标 ID，operation 为 {"kind":"HTTP","http":{"method":"GET","path":"/healthz"}}，reason 为“验证本地 Claude Code 通过 AKV HTTP API 的完整授权链路”。不要添加 query、body、credential_id、目标 URL 或认证头。
5. 告诉我 task_id、request_id、目标名和冻结操作，然后立即暂停。
...
正常情况下，Claude Code 会依次查询目标、建立任务、启动心跳并提交申请，然后显示 `task_id` 和 `request_id`。这两个 ID 是任务和申请标识，不是凭证。Claude Code 暂停回答不等于停止后台心跳。
...
2. 只有 request_status 和 grant_status 都是 APPROVED，且 Grant 尚未过期时，才调用一次执行面的 POST /v1/execute/http；body 只能包含刚才的 request_id 和 task_id。
...
这次演示中，Claude Code 只知道目标的安全元数据、获批操作和执行结果。测试 API Key 由执行代理从 OpenBao 取得并注入请求，不会经过 Prompt 或 Agent 提交的 JSON。该 Grant 只能原子占用一次；即使执行失败，也不能直接重试，必须重新申请并再次人工批准。
...
如果本机曾为旧版本配置过名为 `akv` 的 MCP 连接，可以执行 `claude mcp remove --scope local akv` 清理遗留配置。旧构建留下的 `bin/akv-mcp-server` 也可以直接删除；当前版本不会再构建或使用它。
...
6. Claude Code 是否把 HTTP 申请发送到了 `8080`，把执行请求发送到了 `8081`。
