...
* 自己 Agent 的待审批申请；
* 授权记录和审计时间线；
* 对可见申请显式记录人工转交、审批补问及复盘起止，并查看试点指标；
* 对自己 Agent 授权的主动撤销。

`APPROVE_ALL` 用户额外可查看、审批、撤销全部 Agent 请求，并为这些可见请求记录试点观测。
...
试点观测使用独立的追加式事件，不接受自由文本、客户端时间或 actor 字段。历史申请保持“未知”，采集能力上线后创建的新申请才可将未发生事件表示为明确的 0。观测事件只用于度量，不参与审批、Grant 占用、执行、回收或安全判断。
...
* 审计事件和异常处置记录。
* 不含自由文本的试点观测启用标记及人工转交、审批补问、复盘起止事件。
...
试点观测事件与业务审计分表保存：它们需要严格的枚举结构、复盘起止配对和历史未知/新请求零值区分，但不构成授权事实，也不得被任何授权或执行查询消费。观测记录一经写入不得修改或删除。
