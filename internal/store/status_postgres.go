...
SELECT r.id,r.status,r.approval_deadline,a.decision,g.status,g.expires_at,e.status,rc.status,COALESCE(rc.error_code,e.error_code),r.operation,COALESCE(r.operation_id::text,''),COALESCE(r.operation_version,0)
...
LEFT JOIN LATERAL (SELECT status,error_code FROM reclaims WHERE execution_id=e.id ORDER BY attempt DESC LIMIT 1) rc ON true
	WHERE r.id=$1 AND r.agent_id=$2`, requestID, agentID).Scan(&result.RequestID, &result.RequestStatus, &result.ApprovalDeadline, &decision, &grantStatus, &grantExpires, &executionStatus, &reclaimStatus, &errorCode, &result.OperationKind, &result.OperationID, &result.OperationVersion)
	if err != nil {
