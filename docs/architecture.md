...
| Agent Token 存储 | Agent 运行时直接持有 Bearer Token 并注入 HTTP 认证头；不得写入模型 Prompt、请求 JSON 或日志 |
...
| `POST /v1/agent/authorizations` | 提交目标、强类型操作和申请理由 |
...
执行面接口为 `POST /v1/execute/http`、`POST /v1/execute/postgresql` 和 `POST /v1/execute/sign`。执行 body 只允许 `request_id` 与 `task_id`；执行代理会从服务端冻结快照重新校验操作类型和全部上下文。

Agent Token 只能由可信运行时注入 `Authorization` 请求头，不得出现在模型 Prompt、请求 JSON、项目文件、日志、错误或返回文本中。Bearer 头只能发送到部署时预先配置并验证过的 control 或 execution proxy 精确 Origin；基础地址只允许 `http`/`https`，不得包含 userinfo、非根路径、query 或 fragment，任何认证请求都禁止跟随重定向。Prompt、目标元数据和响应内容不得改变认证请求的 Origin。

直连模式下，Agent 运行时负责按 15 秒间隔发送心跳、禁止对执行请求做透明重试并限制本地记录的响应内容；目标 HTTP 重定向仍由 execution proxy 拒绝。
...
sequenceDiagram
    ...
    Note over Human,Worker: 前置条件：Agent 已注册，可信 Agent 运行时已持有身份 Token；目标系统与源凭证已由唯一管理员配置
    ...
    Agent->>Control: Agent Token + POST /v1/agent/authorizations<br/>task_id、target_id、operation、reason
    ...
    alt 审批拒绝或等待超时
        ...
        Agent->>Proxy: Agent Token + POST /v1/execute/http、postgresql 或 sign<br/>request_id + task_id
...
11. MVP 自助注册只能创建普通用户；管理员身份和 `APPROVE_ALL` 仍只能由既有受控流程管理。
