...
更新：2026-07-15｜总体：`IN_PROGRESS`｜当前：`AKV-003`｜下一项：`AKV-003.a`

## 恢复点

- 核心 schema、迁移器和领域状态转换已完成；状态矩阵测试覆盖所有合法与非法转换。
- 下一轮 `AKV-003.a` 实现人类账户密码哈希、唯一管理员初始化和登录 Session。
- 密码散列按架构使用 bcrypt；只保存散列，登录接口和日志不得包含密码或 Session Token。
...
```text
ID / 目标：AKV-003.a / 实现人类身份核心服务
验收条件：bcrypt 密码哈希；唯一管理员初始化；登录/Session 哈希存储；权限测试；make verify 通过
修改范围：身份领域服务、持久层接口/fake、测试、依赖、memory/progress
验证命令：make verify
风险 / 下一步：敏感输入不得进入日志或持久化明文；并发唯一管理员最终由数据库约束兜底
```
...
| `AKV-003` | `IN_PROGRESS` | 002 | 人类身份、Agent Token、任务与心跳 |
...
- 2026-07-15：`make verify` 和 `git diff --check` 通过；领域状态全矩阵测试证明非法回退和终态重开均被拒绝。
